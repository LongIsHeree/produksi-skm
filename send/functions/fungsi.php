...
function tampilkan_transaksi_all_proses($orc, $style, $status, $costomer, $no_po, $color)
{
  ...
  WHERE B.status = 'jalan' AND C.orc LIKE '%$orc%' AND D.style like '%$style%' AND C.status = '$status' 
    AND C.id_costomer = '$costomer' AND C.no_po LIKE '%$no_po%' AND C.color LIKE '%$color%'
  ORDER BY B.urutan";
...
function tampilkan_preparartion_production($po, $category, $orc, $style, $status, $costomer)
{
  ...
  $query = "SELECT C.orc, C.no_po, C.label, D.style, D.item, C.color, E.costomer, C.qty_order, 
  F.plan_line, F.days_proses, F.plan_production, F.id_prod, F.team_sample_pic, F.team_sample_date, ppm_pic,  ppm_date, pattern_check_date, kesiapan_pattern_check, pattern_check_pic,
  F.template_sewing_pic, F.kesiapan_template_sewing, F.template_sewing_date, F.marker_pic, F.kesiapan_marker, F.marker_date,
  F.moulding_pic, F.moulding_date, F.machines_setting_pic, F.machines_setting_date, F.layout_pic, F.layout_date,
  F.ready_produksi_pic, F.ready_produksi_date, F.fabric_edit_date, F.kesiapan_fabric, F.fabric_pic, F.acc_sewing_edit_date, F.kesiapan_acc_sewing, acc_sewing_pic,
  F.acc_packing_edit_date, F.kesiapan_acc_packing, acc_packing_pic
    FROM  master_order C 
  JOIN style D ON C.id_style = D.id_style
  JOIN costomer E ON C.id_costomer = E.id_costomer 
...
function tampilkan_barang_id($edit)
{
  ...
  $query = "SELECT A.id_style, B.style, A.kode_barcode, A.warna, A.size, A.cup, A.qty_barcode FROM 
...
IFNULL(Q.daily,0) packing_bun_daily, IFNULL(Q.total,0) packing_bun_total, (IFNULL(Q.total,0) -  A.qty_order) packing_bun_balance,
  IFNULL(J.line, 'not_yet') line  FROM 
  (SELECT C.id_order, C.orc, D.style, C.color, E.costomer, C.no_po, A.barcode_bundle, A.id_order_detail, 
   sum(A.qty_isi_bundle) qty_order, C.status, F.category,  C.id_costomer FROM master_bundle A
    JOIN order_detail B ON A.id_order_detail = B.id_order_detail
    JOIN master_order C ON B.id_order = C.id_order
    JOIN style D ON C.id_style = D.id_style
    JOIN costomer E ON C.id_costomer = E.id_costomer
...
--     ON A.id_order = G.id_order    
 ...
 LEFT OUTER JOIN 
  ...
  -- LEFT OUTER JOIN 
  --   (SELECT max(A.tanggal) tanggal_max, C.id_order, sum(if(A.tanggal='$tgl', A.qty, 0)) daily, sum(if(A.tanggal<='$tgl', A.qty, 0)) total FROM transaksi_iron A
  --   JOIN master_bundle B ON A.kode_barcode = B.barcode_bundle
  --   JOIN order_detail C ON B.id_order_detail = C.id_order_detail
  --   WHERE tanggal <= '$tgl' 
  --   GROUP BY C.id_order)K
  --   ON A.id_order = K.id_order	
 
--  LEFT OUTER JOIN 
--     (SELECT max(A.tanggal) tanggal_max, C.id_order, sum(if(A.tanggal='$tgl', A.qty, 0)) daily, sum(if(A.tanggal<='$tgl', A.qty, 0)) total FROM transaksi_qc_kensa A
--     JOIN order_detail B ON A.kode_barcode = B.barcode_number
--     JOIN master_order C ON B.id_order = C.id_order
--     WHERE tanggal <= '$tgl' 
--     GROUP BY C.id_order)L
--     ON A.id_order = L.id_order
         
    -- LEFT OUTER JOIN 
    -- (SELECT max(A.tanggal) tanggal_max, C.id_order, sum(if(A.tanggal='$tgl', A.qty, 0)) daily, sum(if(A.tanggal<='$tgl', A.qty, 0)) total FROM transaksi_tatami A
    -- JOIN order_detail B ON A.kode_barcode = B.barcode_number
    -- JOIN master_order C ON B.id_order = C.id_order
    -- WHERE tanggal <= '$tgl' 
    -- GROUP BY C.id_order)M
    --  ON A.id_order = M.id_order
...
--  LEFT OUTER JOIN 
--     (SELECT max(A.tanggal) tanggal_max, C.id_order, sum(if(A.tanggal='$tgl', A.qty, 0)) daily, sum(if(A.tanggal<='$tgl', A.qty, 0)) total FROM transaksi_kenzin A
--     JOIN order_detail B ON A.kode_barcode = B.barcode_number
--     JOIN master_order C ON B.id_order = C.id_order
--     WHERE tanggal <= '$tgl' 
...
LEFT OUTER JOIN 
    (SELECT max(A.tanggal) tanggal_max, A.tanggal, C.id_order, sum(if(A.tanggal='$tgl', A.qty, 0)) daily, sum(if(A.tanggal<='$tgl', A.qty, 0)) total
     FROM transaksi_qc_buyer A
    JOIN master_bundle B ON A.kode_barcode = B.barcode_bundle
    JOIN order_detail C ON B.id_order_detail = C.id_order_detail
    WHERE tanggal <= '$tgl' 
    GROUP BY C.id_order)P
    ON A.id_order = P.id_order
    ...
    JOIN order_detail C ON B.id_order_detail = C.id_order_detail
    WHERE tanggal <= '$tgl' 
    GROUP BY C.id_order)R
    ON A.id_order = R.id_order    
    

  WHERE A.orc LIKE '%$orc%' AND A.style LIKE '%$style%' AND A.status = '$status' 
    AND A.id_costomer = '$costomer' AND IFNULL(J.line, 'not_yet') LIKE '%$jalan_line%' AND A.no_po LIKE '%$no_po%'
    AND A.color LIKE '%$color%'
    GROUP BY A.id_order
    order by J.tanggal_max desc
    limit 50";
...
function tampilkan_data_preparation_production_id($id)
{
  ...
  $query = "SELECT A.id_prod, A.id_order, D.costomer, B.no_po, B.orc, C.style, C.item, B.color, B.qty_order,  A.plan_line, A.days_proses, A.plan_production,
    A.inhouse_fabric_date, A.kesiapan_fabric_date, A.kesiapan_fabric, A.fabric_pic, A.remaks_fabric,
    A.kesiapan_acc_sewing_date, A.kesiapan_acc_sewing, A.acc_sewing_pic, A.remaks_acc_sewing,
   A.kesiapan_acc_packing_date, A.kesiapan_acc_packing, A.acc_packing_pic, A.remaks_acc_packing,
    A.team_sample_date, A.team_sample_pic, A.remaks_team_sample, A.ppm_date, A.ppm_pic, A.remaks_ppm, 
    A.pattern_check_date, A.kesiapan_pattern_check, A.pattern_check_pic, A.remaks_pattern_check,
    A.template_sewing_date, A.kesiapan_template_sewing, A.template_sewing_pic, A.remaks_template_sewing ,
    A.marker_date, A.kesiapan_marker, A.marker_pic, A.remaks_marker ,
...
function tambah_data_size($size, $kelompok)
{
  $query = "INSERT INTO size (size, kelompok_size) VALUES ('$size', '$kelompok')";
  return run($query);
}
...
function edit_data_barang($barcode, $style, $warna, $size, $cup, $qty_barcode)
{
  global $koneksi;

  $query = "UPDATE barang SET id_style=$style, warna='$warna',
    size='$size', cup = '$cup', qty_barcode = '$qty_barcode' WHERE kode_barcode='$barcode'";
...
function edit_data_preparation_production($id, $pic_pp, $date_in_pp, $date_pp, $username, $kolom_date_in, $kolom_date_transaksi, $kolom_pic_transaksi, $kolom_username_edit, $kolom_user_date_edit, $remaks_kolom, $remaks, $kolom_persentase, $kesiapan, $status_persentase)
{
  ...
  if ($status_persentase == 'ya') {
    ...
    $kolom_date_in = '$date_in_pp' and  $kolom_persentase = '$kesiapan'
    where id_prod = $id";
  } else {
    $query = "UPDATE production_preparation SET $kolom_pic_transaksi='$pic_pp', $kolom_date_transaksi = '$date_pp', $kolom_username_edit = '$username', $kolom_user_date_edit = now(), $remaks_kolom = '$remaks'
    where id_prod = $id";
...
function edit_data_kesiapan_fabric($id, $kesiapan_date, $kesiapan, $pic, $remaks, $username, $inhouse_fabric_date)
{
  ...
  $query = "UPDATE production_preparation SET kesiapan_fabric_date='$kesiapan_date', kesiapan_fabric = $kesiapan, inhouse_fabric_date = '$inhouse_fabric_date', fabric_pic = '$pic', fabric_edit_date = now(), remaks_fabric = '$remaks', fabric_edit_user = '$username'  where id_prod = $id";
...
function edit_data_kesiapan_acc_sewing($id, $kesiapan_date, $kesiapan, $pic, $remaks, $username)
{
  global $koneksi;


  $query = "UPDATE production_preparation SET kesiapan_acc_sewing_date='$kesiapan_date', kesiapan_acc_sewing = $kesiapan, acc_sewing_pic = '$pic', acc_sewing_edit_date = now(), remaks_acc_sewing = '$remaks', acc_sewing_edit_user = '$username'  where id_prod = $id";
