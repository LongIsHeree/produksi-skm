...
function getData($koneksi, $tgl, $status, $orc, $style, $line, $category, $costomer, $no_po) {
    ...
    SUM(A.qty_isi_bundle) qty_order, C.status, F.category, G.plan_line FROM master_bundle A
     ...
     LIMIT 1000) A 
     LEFT OUTER JOIN 
     (SELECT A.tanggal, C.id_order, SUM(IFNULL(A.qty,0)) daily FROM transaksi_cutting A
     ...
     ON A.id_order = B.id_order
      LEFT OUTER JOIN 
     (SELECT MAX(A.tanggal) tanggal_max, C.id_order, SUM(IFNULL(A.qty,0)) output_total FROM transaksi_cutting A
     JOIN master_bundle B ON A.kode_barcode = B.barcode_bundle
     JOIN order_detail C ON B.id_order_detail = C.id_order_detail
     WHERE tanggal <= '$tgl' 
     ...
     LEFT OUTER JOIN
      (SELECT C.id_order, IFNULL(MAX(A.line), 'not_yet') line FROM transaksi_sewing A
      JOIN master_bundle B ON A.kode_barcode = B.barcode_bundle
      JOIN order_detail C ON B.id_order_detail = C.id_order_detail
      GROUP BY C.id_order)AD
     ON A.id_order = AD.id_order 
     WHERE AC.table_transaksi = 'transaksi_cutting'
     $lineFilter
    ";
...
function getBalances($koneksi, $tanggal, $status, $orc, $style, $line) {
    ...
    LEFT JOIN transaksi_cutting C2 ON C2.kode_barcode = MB.barcode_bundle AND C2.tanggal <= '{$tanggal}'
